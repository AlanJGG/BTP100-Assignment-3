...
void searchCallsByCategory(const CallToAction *calls, int numCalls, const char *category) {
    ...
    for (int i = 0; lowerCategory[i]; i++) lowerCategory[i] = tolower(lowerCategory[i]);

    printf("\nSearching for Calls to Action in category '%s':\n", category);
    printf("----------------------------\n");

    for (int i = 0; i < numCalls; i++) {
        strncpy(lowerCallCategory, calls[i].category, sizeof(lowerCallCategory) - 1);
        lowerCallCategory[sizeof(lowerCallCategory) - 1] = '\0';
        for (int j = 0; lowerCallCategory[j]; j++) lowerCallCategory[j] = tolower(lowerCallCategory[j]);
